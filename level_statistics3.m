...
for i=1:size(sector_considered,1)
    ...
    disp("bases found")
    bases_sector_odd = bases_sector(1:2:end,:);
    bases_sector_even = bases_sector(2:2:end,:);

    Hspin1_bond = Hspin1_bond_fun(bases_sector_odd,bases_sector_even,bases_fourier_sector)
    ...
    Hspin2_bond = Hspin2_bond_fun(bases_sector_odd,bases_sector_even,bases_fourier_sector);
    ...
    Nsites_par = Nsites;
    Nparticles1_par = Nparticles1;
    Nparticles2_par = Nparticles2;
    T1_hop_par = T1_hop;
    T2_hop_par = T2_hop;
    omega_par = omega;
    for i_par = 1:Nsites_par
        momentum = i_par - 1
        if size(bases_fourier_sector.num(bases_fourier_sector.num<Nsites_par),2)>0
            states_momentum=States_momentum(bases_fourier_sector,momentum);
        else
            states_momentum=1:size(bases_fourier_sector.num,2);
        end
        %states_momentum
        H_fourier_spin1=Hamiltonian_fourier(momentum,states_momentum,hbonds_translation_spin1,bases_fourier_sector,1,T1_hop_par,T2_hop_par,omega_par);
        H_fourier_spin2=Hamiltonian_fourier(momentum,states_momentum,hbonds_translation_spin2,bases_fourier_sector,2,T1_hop_par,T2_hop_par,omega_par);
        H_fourier=H_fourier_spin1+H_fourier_spin2;
        dimH_m = size(H_fourier,1);
...
%         hi=histogram(r_ediff,'Normalization','pdf');
        xmax_plot=5;
        f1 = figure;
        histogram(e_diff,0:0.2:xmax_plot,'Normalization','pdf');
%       hi.BinWidth=binwidth;
        ...
        hold off
        title(['level stat, sector: ',num2str(sector), '(', num2str([Nsites_par, Nparticles1_par, Nparticles2_par]),'),', ' momentum= ', num2str(momentum), '(',num2str(dimH_m),')'])
        saveas(f1,[pwd '/plots/1688_parallel/without inv sym/level stat/' num2str(sector) '_p=' num2str(momentum) '.pdf']);
        delete(f1)
    end
...
function Hspin1_bond = Hspin1_bond_fun(stateMatrix_odd,stateMatrix_even,bases_fourier_sector)
% bases_fourier_sector is of class BasesFourier
dimH = size(stateMatrix_odd,1);
Hspin1_bond = {};
max_iter = size(bases_fourier_sector.num,2);
bases_fourier_sector_states = bases_fourier_sector.states;

for i=1:max_iter
    hbondi = [];
    statesi = bases_fourier_sector_states{i};
    statesi_1 = statesi(1);
    
    col = statesi_1;
    %state_c_spin2=stateMatrix(2*(col-1)+1,:);
    state_c_spin2=stateMatrix_odd(col,:);
    %state_c_spin1=stateMatrix(2*col,:);
    state_c_spin1=stateMatrix_even(col,:);
    for row=1:dimH
        %state_r_spin2=stateMatrix(2*(row-1)+1,:);
        state_r_spin2=stateMatrix_odd(col,:);
        %state_r_spin1=stateMatrix(2*row,:);
        state_r_spin1=stateMatrix_even(col,:);
        Hrc=H_spin1Hop(state_r_spin1,state_r_spin2,state_c_spin1,state_c_spin2);
        if Hrc ~= 0
            hbondi = [hbondi, row];
        end
    end
    Hspin1_bond{i} = hbondi;
end
end



function Hspin2_bond = Hspin2_bond_fun(stateMatrix_odd,stateMatrix_even,bases_fourier_sector)
% bases_fourier_sector is of class BasesFourier
dimH = size(stateMatrix_odd,1);
Hspin2_bond = {};
max_iter = size(bases_fourier_sector.num,2);
...
parfor i=1:max_iter
    ...
    for row=1:dimH
        ...
        %state_r_spin1=stateMatrix(2*row,:);
        state_r_spin1=stateMatrix_even(col,:);
        Hrc=H_spin2Hop(state_r_spin1,state_r_spin2,state_c_spin1,state_c_spin2);
        if Hrc ~= 0
            hbondi = [hbondi, row];
        end
    end
    Hspin2_bond{i} = hbondi;
end
end
...
for ind_states=1:dim
     ...
     statei=statei_f(1);
     bondi=H_bond{ind_states};
     bondi=bondi(bondi>statei);
...
function H_fourier=Hamiltonian_fourier(momentum,states_momentum,hbonds_translation,bases_f,spin,T1_hop,T2_hop,omega)
