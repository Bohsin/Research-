...
for i=1:size(sectors,1)
    sectori=sectors(i,:)
    indspin1_start=1;
    states_sectori=States_sector(indspin1_start, sectori);
    H=Hamiltonian_spin1(states_sectori)+Hamiltonian_spin2(states_sectori);
    energy=eig(H);
    e_diff=diff(energy);
    binwidth=min(nonzeros(e_diff))/10;
    figure(fignum)
    fignum=fignum+1;
    hi=histogram(energy);
    hi.BinWidth=binwidth;
    title(['degeneracy for the sector ',num2str(sectori), ' (', num2str([Nsites, Nparticles1, Nparticles2]),')'])
%    title(['level statistics for the sector ',num2str(sectori), ' (', num2str([Nsites, Nparticles1, Nparticles2]),')'])
    total_states_sectori=size(states_sectori,1)/2;
    total_states=total_states + total_states_sectori;
end
total_states
nchoosek(Nsites,Nparticles1)*nchoosek(Nsites,Nparticles2)
